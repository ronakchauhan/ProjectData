...
namespace Ktpl\Blog\Model\ResourceModel\Post\Wishlist;
...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection
{
    ...
    {
        parent::_construct();
        $this->_init('Ktpl\Blog\Model\Post', 'Ktpl\Blog\Model\ResourceModel\Post');
     ...
     * @param array|int|\Ktpl\Blog\Model\Category  $category
     * @return $this
     */
    public function addCategoryFilter($category)
    {
        if (!$this->getFlag('category_filter_added')) {
            if ($category instanceof \Ktpl\Blog\Model\Category) {
     ...
     * @param array|int|\Ktpl\Blog\Model\Tag  $tag
     * @return $this
     */
    public function addTagFilter($tag)
    {
        if (!$this->getFlag('tag_filter_added')) {
            if ($tag instanceof \Ktpl\Blog\Model\Tag) {
                $tag = [$tag->getId()];
            }
     ...
     * @param array|int|\Ktpl\Blog\Model\Author  $author
     * @return $this
     */
    public function addAuthorFilter($author)
    {
        if (!$this->getFlag('author_filter_added')) {
            if ($author instanceof \Ktpl\Blog\Model\Author) {
                $author = [$author->getId()];
            }
    ...
    protected function _afterLoad()
    {
        ...
        if (count($items)) {
            ...
            $tableName = $this->getTable('ktpl_blog_post_store');
            ...
            foreach ($map as $key => $property) {
                $tableName = $this->getTable('ktpl_blog_post_' . $key);
                $select = $connection->select()
                    ->from(['cps' => $tableName])
                    ->where('cps.post_id IN (?)', $items);
    ...
    protected function _renderFiltersBefore()
    {
        foreach(['store', 'category', 'tag'] as $key) {
            if ($this->getFilter($key)) {
                $this->getSelect()->join(
                    [$key.'_table' => $this->getTable('ktpl_blog_post_'.$key)],
