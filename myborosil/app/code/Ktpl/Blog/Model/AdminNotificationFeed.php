...
class AdminNotificationFeed extends \Magento\AdminNotification\Model\Feed
{
    ...
    public function getFeedUrl()
    {
        ...
        $modulesParams = [];
        foreach($this->getKtplModules() as $key => $module) {
            $key = str_replace('Ktpl_', '', $key);
    ...
    /**
     * Get Ktpl Modules Info
     *
     * @return $this
     */
    protected function getKtplModules()
    {
        $modules = [];
        foreach($this->_moduleList->getAll() as $moduleName => $module) {
            if ( strpos($moduleName, 'Ktpl_') !== false && $this->_moduleManager->isEnabled($moduleName) ) {
    ...
    {
        return $this->_cacheManager->load('ktpl_admin_notifications_lastcheck');
    }
    ...
    {
        $this->_cacheManager->save(time(), 'ktpl_admin_notifications_lastcheck');
        return $this;
    }
